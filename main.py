        ...
        with st.form("registration_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                # Validation 2: Check index number format (STUBTECH + 6 digits)
                    ...
                    st.warning("Your input: " + index_number)
...
def main():
    ...
    with tab1:
        ...
        with st.form("registration_form", clear_on_submit=True):
            name = st.text_input("Full Name*", placeholder="Enter your full name", max_chars=100)
            index_number = st.text_input("Index Number*", placeholder="Enter your index number", max_chars=20).strip().upper()
            
            # Real-time validation hint
            if index_number and index_number in df['index_number'].values:
                st.warning("⚠️ This index number is already registered!")
            ...
            if submitted:
                ...
                # Validation: Check for duplicate index number
                elif index_number in df['index_number'].values:
                    existing_student = df[df['index_number'] == index_number].iloc[0]
                    st.error(f"❌ **Duplicate Entry Detected!**")
                    st.warning(f"Index number **{index_number}** is already registered to **{existing_student['name']}**")
                ...
                # Validation: Check for invalid characters in index number
                elif not index_number.replace('-', '').replace('/', '').isalnum():
                    st.error("❌ Index number can only contain letters, numbers, hyphens, and slashes!")
